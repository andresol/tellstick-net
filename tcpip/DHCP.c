...
#define DHCP_DISCOVERY_TIMEOUT			(2ul*TICK_SECOND)
#define DHCP_REQUEST_TIMEOUT				(15ul*TICK_SECOND)
...
typedef struct
{
	...
	DWORD 				dwSecs;		// The secs value in DHCP requests
	...
	for(i = 0; i < NETWORK_INTERFACES; i++)
	{
		...
		switch(DHCPClient.smState)
		{
			...
			case SM_DHCP_SEND_DISCOVERY:
				...
				if (DHCPClient.dwSecs == 0) {
					DHCPClient.dwSecs = TickGet();
				}
			...
			case SM_DHCP_GET_OFFER:
				...
				if(UDPIsGetReady(DHCPClient.hDHCPSocket) < 250u)
				{
					...
					if(TickGet() - DHCPClient.dwTimer >= DHCP_DISCOVERY_TIMEOUT) {
						DHCPClient.smState = SM_DHCP_SEND_DISCOVERY;
						DHCPClient.tempIPAddress.Val = 0x0;
					}
					break;
				}
				...
				DHCPClient.dwSecs = 0; // Reset the secs timer
				// No break
	
			case SM_DHCP_SEND_REQUEST:
			case SM_DHCP_SEND_REQUEST2:
			case SM_DHCP_SEND_REQUEST3:
			case SM_DHCP_SEND_REQUEST4:
			...
			case SM_DHCP_GET_REQUEST_ACK2:
			case SM_DHCP_GET_REQUEST_ACK3:
			case SM_DHCP_GET_REQUEST_ACK4:
				// Check to see if a packet has arrived
				if(UDPIsGetReady(DHCPClient.hDHCPSocket) < 250u)
				{
					// Go back and transmit a new request if we didn't get an ACK after 15 seconds
					if(TickGet() - DHCPClient.dwTimer >= DHCP_REQUEST_TIMEOUT) {
						if(++DHCPClient.smState > SM_DHCP_GET_REQUEST_ACK4) {
							// Fourth retry failed, go back to discovery
							DHCPClient.smState = SM_DHCP_SEND_DISCOVERY;
						}
					}
					break;
...
static void _DHCPSend(BYTE messageType, BOOL bRenewing)
{
	...
	DWORD s = 0;
	if (DHCPClient.dwSecs) {
		s = (TickGet() - DHCPClient.dwSecs) / TICK_SECOND;
	}
	...
	UDPPut(0x34);                               // xid[2]
	UDPPut(0x56);                               // xid[3]
	UDPPut(((BYTE*)(&s))[1]);                   // secs[0]
	UDPPut(((BYTE*)(&s))[0]);                   // secs[1]
